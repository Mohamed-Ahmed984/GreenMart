...
class _MainAppScreenState extends State<MainAppScreen> {
  ...
  Widget _account() => Scaffold(
    ...
    body: ListView(padding: const EdgeInsets.all(24), children: [
      ...
      const SizedBox(height: 16),
      const Text('Your basket and favorites stay available while this session is open. Leaving the demo or restarting the app clears them.'),
  ...
  @override
  Widget build(BuildContext context) => AnimatedBuilder(animation: _store,
    builder: (context, _) => Scaffold(
      body: IndexedStack(index: _index, children: [
        HomeScreen(store: _store),
        SearchScreen(store: _store),
        CartScreen(store: _store, onBrowse: () => setState(() => _index = 0)),
        Scaffold(appBar: AppBar(title: const Text('Saved favorites')),
          body: ProductGrid(products: _store.favorites, store: _store,
            emptyMessage: 'No favorites yet. Tap a heart on any product to save it.')),
        _account(),
      ]),
      bottomNavigationBar: BottomNavigationBar(type: BottomNavigationBarType.fixed,
        currentIndex: _index, onTap: (index) => setState(() => _index = index),
        selectedFontSize: 11, unselectedFontSize: 11,
        items: [
          const BottomNavigationBarItem(icon: Icon(Icons.storefront_outlined), label: 'Home'),
          const BottomNavigationBarItem(icon: Icon(Icons.search), label: 'Explore'),
          BottomNavigationBarItem(icon: Badge(isLabelVisible: _store.itemCount > 0,
            label: Text('${_store.itemCount}'), child: const Icon(Icons.shopping_basket_outlined)), label: 'Cart'),
