        ...
        children: [
          Expanded(child: Stack(fit: StackFit.expand, children: [
            ...
            Align(alignment: Alignment.topRight, child: IconButton(
              ...
              tooltip: store.isFavorite(product.id) ? 'Unsave ${product.name}' : 'Save ${product.name}',
              onPressed: () => store.toggleFavorite(product.id),
              icon: Icon(store.isFavorite(product.id) ? Icons.favorite : Icons.favorite_border,
          ...
          Text(product.quantity, style: Theme.of(context).textTheme.bodySmall),
          const SizedBox(height: 8),
          Row(children: [
            Expanded(child: Text(money(product.priceCents), maxLines: 1,
              overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w600))),
            IconButton.filled(key: Key('add-${product.id}'), tooltip: 'Add ${product.name}',
              onPressed: () {
                final added = store.add(product.id);
                ScaffoldMessenger.of(context)
                  ..hideCurrentSnackBar()
                  ..showSnackBar(SnackBar(duration: const Duration(seconds: 1),
                    content: Text(added ? '${product.name} added to basket' : 'Maximum quantity is 99')));
...
class ProductGrid extends StatelessWidget {
  const ProductGrid({super.key, required this.products, required this.store,
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      final count = ((constraints.maxWidth - 32) / 180).floor().clamp(1, 4).toInt();
      ...
      return GridView.builder(
        ...
        gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: count,
          mainAxisSpacing: 14, crossAxisSpacing: 14, mainAxisExtent: 290 + (scale - 1).clamp(0, 3).toDouble() * 100),
        itemBuilder: (_, index) => ProductCard(product: products[index], store: store),
      );
    });
  }
}
